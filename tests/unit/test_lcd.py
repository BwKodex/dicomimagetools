...
TEST_MATRIX = np.asarray([[2, -7, 16, -3, -1, -15, 12, 5, 19, -2, -3, 13, -6, 19, 9, 3, -14, 12, 14, -11],
...
def test_lcd_statistical_random_raises_analysis_matrix_typeerror():
    with pytest.raises(TypeError):
        lcd_statistical_random(analysis_matrix='Invalid argument type', pixel_size=VoxelData(x=1.0, y=1.0),
                               object_size=1.0)


def test_lcd_statistical_random_raises_pixel_size_typeerror():
    with pytest.raises(TypeError):
        lcd_statistical_random(analysis_matrix=TEST_MATRIX, pixel_size='Invalid argument type',
                               object_size=1.0)


def test_lcd_statistical_random_raises_object_size_typeerror():
    with pytest.raises(TypeError):
        lcd_statistical_random(analysis_matrix=TEST_MATRIX, pixel_size=VoxelData(x=1.0, y=1.0),
                               object_size='Invalid argument type')


def test_lcd_statistical_random_raises_rois_typeerror():
    with pytest.raises(TypeError):
        lcd_statistical_random(analysis_matrix=TEST_MATRIX, pixel_size=VoxelData(x=1.0, y=1.0),
                               object_size=1.0, rois='Invalid argument type')
